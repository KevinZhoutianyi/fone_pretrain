...
|---|---|---|---|---|
| 1987, 1988 (forced), 1989, 1990 (mean_ctrl) | olmo_forced / olmo_meanctrl | running (4 runs parallel, 4 nodes each) | §3 | FORCE the model to use FoNE. The fone arm let the model route around the code (its scale decayed toward 0: pretrained number rows keep their identity in the free non-code dims, so the code is optional). fone_forced removes that escape hatch: every number row = a FROZEN shared mean (over pretrained number rows) + FoNE code on the code_dims, so two numbers differ ONLY in the code -> the model must read it. mean_ctrl is the control: same frozen shared mean, NO code (all number tokens share one embedding), isolating the code from the mean-collapse. 46 code dims. FALSIFIER: if fone_forced <= mean_ctrl on full-1319 GSM8K, the code adds nothing beyond collapsing number rows. Smoke (1986) passed: emb code scale ROSE over 20 steps (0.2973->0.2980) vs the free-dim fone arm's decay -- early sign the force holds. ~16h/run. Compare to baseline (full-set 0.272). |
...
| olmo stage-2 wave (train 1621+, eval full 1847-1850) | olmo_stage2_surgery | §3 | FoNE embedding surgery on pretrained OLMo-2-1B, stage-2 continued pretrain on 40.6B/50B Dolmino tokens (81% of official; near-official mix dclm 46%/math 20.9%), then GSM8K greedy exact-match on the **full 1319-question test set**. 2 arms x 2 seeds all trained to ckpt_final. RESULT: **TIED.** fone mean 0.277 (s1337 0.2805, s2024 0.2737) vs baseline mean 0.272 (s1337 0.2889, s2024 0.2555); gap +0.5pt, but within-arm seed spread (baseline 0.256-0.289, 3.4pt) exceeds it -- 2 seeds cannot separate the arms. Baseline s1337 (0.289) is the single best of all four. NOTE: an earlier n=200 read showed fone +1.75pt (0.273 vs 0.255); the full set killed it -- n=200 was a slice where baseline s1337 happened to score 0.255 vs 0.289 on the full set. Same tie verdict as the 125M compare. Method change (high-bandwidth fone) launched to open a margin. |
...
| 1883, 1884 | olmo_fone_hi (high-bandwidth) | scancel'd at ~48% (step ~9400/19359). Not a crash: the diagnosis was already visible. On the real run the fone code scale DECAYED (head_scale 0.06 init -> 0.0035 by step 9400, emb 0.30 -> 0.077), i.e. the model was turning the code OFF -- it reads number identity from the free non-code dims of the pretrained embedding (an in-distribution local optimum) and ignores the injected code. More bandwidth (20->46 dims) gives more dims to ignore, not a fix. | Killed to reallocate 8 nodes. Root cause is the escape hatch (free non-code number dims), not bandwidth. Replaced by fone_forced (freeze number rows to a shared mean so the code is the ONLY number signal) + mean_ctrl (see Active runs). Config-driven bandwidth kept in code; the fone_hi configs remain for the record. |
